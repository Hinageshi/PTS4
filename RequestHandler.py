...
class RequestHandler:
    ...
    def getSampleRandomAgents(self, numberOfAgents):
        ...
        result = json.loads(urllib.request.urlopen(url).read().decode())
        for agent in result:
           sample.append(Agent(agent['id'], agent['country_name'], agent['latitude'], agent['longitude'], agent['sex'], agent['date_of_birth'],
                              agent['age'], agent['language'], agent['religion'], agent['income'], agent['internet'], agent['openness'],
                              agent['conscientiousness'], agent['extraversion'], agent['agreeableness'], agent['neuroticism'], agent['id_str']))
        return sample
